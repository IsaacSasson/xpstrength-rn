...
export async function workoutPlanValidator(plan) {
    ...
    for (const [index, element] of plan.entries()) {
        if (!Number.isInteger(element) || !Number.isFinite(element) || Number.isNaN(element)) {
        ...
        const plannedWorkout = await CustomWorkout.findOne({ where: { id: element, userId: this.userId } });
        if (!plannedWorkout) {
            throw new Error("Planned Workout does not exist in users planned workouts.")
        }
