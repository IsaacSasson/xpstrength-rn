...
import QuickActionsCustomizer, { ActionDefinition } from "@/components/home/QuickActionsCustomizer";
...
const processExerciseData = (ex: any, exerciseDatabase: any[]) => {
  const exerciseDetails = exerciseDatabase.find((e) => e.id === (ex.exercise ? String(ex.exercise) : ""));
  ...
  if (Array.isArray(ex.sets)) {
    ...
    return { name: exerciseName, sets: setsCount, reps: repsDisplay };
  } else {
    return { name: exerciseName, sets: ex.sets || 0, reps: String(ex.reps || 0) };
  }
};
...
export default function Home() {
  ...
  /* preload vector icon fonts so first render never blocks */
  useEffect(() => {
    (async () => {
      try {
        await Promise.all([FontAwesome5.loadFont(), MaterialCommunityIcons.loadFont()]);
      } catch {}
    })();
  }, []);
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (workout) {
        workoutMap[key] = {
          exists: true,
          name: workout.name,
          calories: Math.round(workout.exercises.length * 50 + Math.random() * 100),
          exercises: workout.exercises.slice(0, 4).map((ex: any) => processExerciseData(ex, exerciseDatabase)),
        };
      }
    }
    return workoutMap;
  ...
  const allActions: ActionDefinition[] = useMemo(
    () => [
      { id: "createWorkout", title: "Create New Workout", icon: "dumbbell", iconType: "material", onPress: goToCreateWorkout },
      { id: "viewPlan", title: "Weekly Plan", icon: "calendar-week", iconType: "fontawesome", onPress: goToPlannedWorkouts },
      { id: "workoutPlans", title: "Workout Plans", icon: "clipboard-list", iconType: "fontawesome", onPress: goToWorkoutPlans },
      { id: "inviteFriends", title: "Invite Friends", icon: "user-plus", iconType: "fontawesome", onPress: goToFriends },
      { id: "viewProgress", title: "View Progress", icon: "chart-line", iconType: "fontawesome", onPress: goToStats },
      { id: "exerciseList", title: "Exercise List", icon: "list", iconType: "fontawesome", onPress: () => router.push("/home/exercise-list") },
      { id: "personalBests", title: "Personal Bests", icon: "trophy", iconType: "fontawesome", onPress: () => router.push("/stats/personal-bests") },
      { id: "goals", title: "Goals", icon: "medal", iconType: "fontawesome", onPress: () => router.push("/stats/achievements") },
      { id: "settings", title: "Settings", icon: "cog", iconType: "fontawesome", onPress: () => router.push("/profile/settings") },
    ],
    [goToCreateWorkout, goToPlannedWorkouts, goToWorkoutPlans, goToFriends, goToStats]
  );
  ...
  const idToAction = useCallback((id: string) => allActions.find((a) => a.id === id)!, [allActions]);
  const currentSlots = quickSlotIds.filter(Boolean).map((id) => idToAction(id as string));

  const handleRetry = () => { clearError(); refreshData(); };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#0F0E1A" }}>
      ...
      <ScrollView showsHorizontalScrollIndicator={false} className="px-4 pb-6">
        ...
        <Calender selectedDate={selectedDate} onSelectDate={setSelectedDate} restDays={[]} />
        ...
        <View className="flex-row items-center justify-between mt-4 mb-6">
          ...
          <TouchableOpacity onPress={goToWorkoutPlans} className="flex-row items-center">
            <Text style={{ color: primaryColor }} className="mr-2 font-pmedium">My Workouts</Text>
            <FontAwesome5 name="clipboard-list" size={16} color={primaryColor} />
          </TouchableOpacity>
        </View>

        {isLoading && (
          <View className="rounded-2xl p-8 mb-6 items-center" style={{ backgroundColor: tertiaryColor }}>
        ...
        {error && !isLoading && (
          <View className="rounded-2xl p-6 mb-6" style={{ backgroundColor: "#ff4d4d20" }}>
            <Text className="text-red-400 font-pmedium text-center">{error}</Text>
            <TouchableOpacity onPress={handleRetry} className="mt-3 items-center">
              <Text style={{ color: primaryColor }} className="font-pmedium">Tap to retry</Text>
            </TouchableOpacity>
          </View>
        )}

        {!isLoading && !error && (
          <TodaysWorkout workout={workoutForDate} allowCreate={! (selectedDate < midnight && !workoutForDate)} selectedDate={selectedDate} />
        )}
